...
match secim:
    ...
    case "2":
        ...
        sorgu = "SELECT filmID, filmAdi, yonetmen FROM filmbilgi"
        imlec.execute(sorgu)
        ...
        for veri in veriler:
            ...
            yonetmenID=veri[2]
            sorgu1 = f"SELECT yonetmenAdi FROM yonetmen WHERE yonetmenID={yonetmenID}"
            imlec1.execute(sorgu1)
            veriler1 = imlec1.fetchall()
            for veri1 in veriler1:
                yonetmenAdi=veri1[0]
            tablo.add_row([veri[0],veri[1],yonetmenAdi])
        tablo.align = "l"
        print(tablo)
        imlec.close()
