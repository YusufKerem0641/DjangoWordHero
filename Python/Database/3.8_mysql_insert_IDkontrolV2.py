...
while True:
    ...
    # Bu kayıt kişiler tablosunda var mı?
    sorgu = f"SELECT * FROM kisi WHERE kisiID={id}"
    imlec.execute(sorgu)
    kontrolVerisi = imlec.fetchall()
    if kontrolVerisi:
        print ("Bu kayıt için kişi tablosunda veri bulunmaktadır.")
        continue
    break
...
while True:
    ...
    if TCkno_kontrol(tckno):
        print("T.C. kimlik numarası geçerlidir...")
